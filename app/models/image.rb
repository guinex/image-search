class Image < ApplicationRecord
    ...
    if (images = get_image_for_design_from_readonly(design_ids)) 
      images.each do |image|
        if (file_path = check_if_url_exists(image[:id], image[:photo_file_name])).present?
          ...
          result = Image.where(fingerprint: fingerprint)
        end
      end
    end  
    result
  ...
  #tested
  def self.automated_feed_new_images
  ...
  def self.re_arrange_equal_images
    images = Image.where('processed_for_duplicate_at is null or processed_for_duplicate_at >= ?',1.weeks.from_now).order("RANDOM()").limit(1000)
    if images.present? 
      image_id_processed = []
      images.each do |image|
        unless image_id_processed.include?(image.id)
          equal_images = Image.where(fingerprint: image.fingerprint)
          ...
          if equal_image_hash.present? && equal_image_hash.count > 1
            ...
            Image.delete(equal_image_ids)
            image.save!
          end
        end  
      end
      images.update_all(processed_for_duplicate_at: Time.zone.now())
    end  
  end
  ...
  def self.automated_update_similar_designs
    images = Image.where('processed_for_similar_at is null or processed_for_similar_at >= ?',1.weeks.from_now).order("RANDOM()").limit(10)
    if images.present?
    ...
    similar_images.each do |similar|
      if similar[1].present?
        if id_design_ids[1].include? similar[0]
          ...
          if (image = Image.where(id: image_id).first).present?
  ...
  def self.get_master_images_for_image_search
    image_array = []
    query = "SELECT id, design_id, photo_file_name FROM images WHERE kind = 'master' and photo_file_name is not null and design_id is not null LIMIT 1000"
    connection = ReadonlyConnection.connect
    results = connection.execute(query)
    connection.disconnect!
    ReadonlyConnection.reset_connection
